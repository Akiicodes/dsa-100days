...
int evalRPN(char** tokens, int tokensSize) {
  ...
  34: {
    github: `/*Problem: Evaluate Postfix Expression - Implement using linked list with dynamic memory allocation.
...
`
  ...
  35: {
    github: `/*Problem: Queue Using Array - Implement using linked list with dynamic memory allocation.
...
`
    ...
    36: {
    github: `  /*Problem: Circular Queue Using Array - Implement using linked list with dynamic memory allocation.
...
`
  ...
  38: {
    github: `  /*Problem: Deque (Double-Ended Queue)
...
`,
    leetcode: `int maxDepth(struct TreeNode* root) {
    if(root == NULL) return 0;
...
`
  ...
  47: {
    github: `/*Problem: Height of Binary Tree
...
`,
    leetcode: `NO QUESTIONS FOUND!
}`
  },
  50: {
    github: `/*Problem: BST Search
...
`,
    leetcode: `struct TreeNode* searchBST(struct TreeNode* root, int val) {
...
`,
    leetcode: `struct TreeNode* lowestCommonAncestor(struct TreeNode* root, struct TreeNode* p, struct TreeNode* q) {
    if (root == NULL)
        return NULL;
...
int main() {
    ...
    leetcode: `int** zigzagLevelOrder(struct TreeNode* root, int* returnSize, int** returnColumnSizes) {
    if (root == NULL) {
        *returnSize = 0;
        return NULL;
    }
...
int main() {
    ...
    leetcode: `int* rightSideView(struct TreeNode* root, int* returnSize) {
    if (root == NULL) {
        *returnSize = 0;
        return NULL;
    }
...
int main() {
    ...
    leetcode: `int isMirror(struct TreeNode* t1, struct TreeNode* t2) {
    if (t1 == NULL && t2 == NULL)
        return 1;
    if (t1 == NULL || t2 == NULL)
        return 0;
...
int main() {
    ...
    leetcode: `void helper(struct TreeNode* root, struct TreeNode** prev) {
    if (root == NULL)
        return;
...
struct TreeNode* helper(int* preorder, int* inorder, int start, int end, int* preIndex) {
    if (start > end)
        return NULL;
    ...
    root->left = helper(preorder, inorder, start, inIndex - 1, preIndex);
    root->right = helper(preorder, inorder, inIndex + 1, end, preIndex);
...
bool canVisitAllRooms(int** rooms, int roomsSize, int* roomsColSize) {
  ...
  63: {
    github: `/*Problem: Perform DFS starting from a given source vertex using recursion.
...
int main() {
    ...
    leetcode: `void dfs(int** image, int m, int n, int r, int c, int oldColor, int newColor) {
    if (r < 0 || c < 0 || r >= m || c >= n)
        return;
...
int main() {
  ...
  65: {
    github: `/*Problem: Using DFS and parent tracking, detect if undirected graph has a cycle.
...
bool canFinish(int numCourses, int** prerequisites, int prerequisitesSize, int* prerequisitesColSize) {
  ...
  67: {
    github: `/*Problem: Print topological ordering of a Directed Acyclic Graph (DAG) using DFS.*/
#include <stdio.h>
#include <stdlib.h>
...
int main() {
    ...
    leetcode: `int* findOrder(int numCourses, int** prerequisites, int prerequisitesSize, int* prerequisitesColSize, int* returnSize) {
    int** adj = (int**)malloc(numCourses * sizeof(int*));
    int* adjSize = (int*)calloc(numCourses, sizeof(int));
...
public:
  ...
  69: {
    github: `//Problem: Given weighted graph with non-negative edges, compute shortest path from source using priority queue./*
//Solution:
#include <stdio.h>
#include <stdlib.h>
#include <limits.h>
...
public:
    ...
    void dfs(int u, int parent, vector<int> adj[], vector<int>& vis,
             vector<int>& disc, vector<int>& low, vector<int>& mark) {
    ...
    vector<int> articulationPoints(int V, vector<int> adj[]) {
        vector<int> vis(V, 0), disc(V), low(V), mark(V, 0);
        timer = 0;
...
int findTheCity(int n, int** edges, int edgesSize, int* edgesColSize, int distanceThreshold) {
  ...
  81: {
    github: `/*Problem: Implement Bubble Sort - Implement the algorithm.
...
`,
  ...
  82: {
    github: `/*Problem: Given a sorted array of integers and a target value x, implement the Lower Bound and Upper Bound operations using Binary Search.
...
`,
    leetcode: `int searchInsert(int* nums, int numsSize, int target) {
...
`,
  ...
  84: {
    github: `/*Problem: Implement Insertion Sort - Implement the algorithm.
...
`,
    leetcode: `int findPeakElement(int* nums, int numsSize) {
    int left = 0, right = numsSize - 1;
  ...
  85: {
    github: `/*Problem: Implement Merge Sort - Implement the algorithm.
...
`,
    leetcode: `int findMin(int* nums, int numsSize) {
    int left = 0, right = numsSize - 1;
  ...
  86: {
    github: `/*Problem: Find the integer square root of a given non-negative integer using Binary Search. The integer square root of a number is defined as the greatest integer whose square is less than or equal to the given number.
...
int splitArray(int* nums, int numsSize, int k) {
  ...
  91: {
    github: `/*Problem: Implement merge sort (divide, sort halves, merge).
Print sorted array.*/
//Solution:
#include <stdio.h>
...
`,
    leetcode: `void sortColors(int* nums, int numsSize) {
...
struct ListNode* insertionSortList(struct ListNode* head) {
  ...
  94: {
    github: `/*Problem: Sort array of non-negative integers using counting sort.
...
`,
    leetcode: `int* relativeSortArray(int* arr1, int arr1Size, int* arr2, int arr2Size, int* returnSize) {
    int count[1001] = {0};
...
int reversePairs(int* nums, int numsSize) {
  ...
  97: {
    github: `/*Problem: Given meeting intervals, find minimum number of rooms required.
...
public:
  ...
  98: {
    github: `/*Problem: Given intervals, merge all overlapping ones.
...
int** merge(int** intervals, int intervalsSize, int* intervalsColSize,
    ...
    *returnColumnSizes = (int*)malloc(intervalsSize * sizeof(int));
    ...
    for (int i = 1; i < intervalsSize; i++) {
        ...
        if (intervals[i][0] <= result[idx - 1][1]) {
